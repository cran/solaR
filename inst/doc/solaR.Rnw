...
The current version of this package allows the whole calculation from both \emph{daily} and 
...
Besides, the package includes a tool for the statistical analysis of
...
The current version of  \texttt{solaR} includes several correlations 
(type \texttt{help(corrFdKt)} for details). 
...
Let's use \texttt{corr='user'} to define a function with the correlation of Page. 
...
On the other hand, the function  \texttt{readMAPA} is able to download the meteorological data 
...
With this version, \texttt{solaR} accepts intradaily irradiation data sources. 

For example, the Measurement and Instrumentation Data Center of the NREL (NREL-MIDC) provides meteorological data from a variety of stations. We will try the \emph{La Ola - Lanai} station at Hawaii (\url{http://www.nrel.gov/midc/la_ola_lanai/}).
<<echo=TRUE>>=
...
If these components were not available, a fd-kt hourly correlation is needed (figure \ref{fig:FdKtBRL}). For example:
...
\begin{centering}
<<echo=TRUE, fig=TRUE,width=6>>=
p<-xyplot(Gef/G~cosTheta|month, data=gef, type=c('p', 'smooth'), par.settings=custom.theme(pch=19, alpha=0.5, cex=0.4))
print(p)
@
\par\end{centering}
...
The next lines of code calculate the movement of a N-S horizontal axis tracker with \emph{backtracking} and whose inclination angle is limited to $\ang{60}$. The evolution of the inclination angle is displayed in the figure \ref{fig:Backtracking}. 
...
Finally, we can compare these systems with the method
\texttt{compare} (fig. \ref{fig:compare}), and calculate and compare their losses with the
methods \texttt{losses} and \texttt{compareLosses} (fig. \ref{fig:compareLosses}), respectively.
